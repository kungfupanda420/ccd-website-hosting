...
const db = mysql.createConnection({
  ...
  password: "Pratheek@1234", // Replace with your MySQL password
...
app.post(
  ...
  (req, res) => {
    ...
    const query = `
      INSERT INTO users (
        name, phone, dob, address, state, guardianName, guardianRelation, guardianPhone,
        email, password, profilePhoto, institution, program, department, year,
        instituteLocation, instituteState, currentSemesterCgpa, UG, cgpa12, board12, cgpa10, board10,
        paymentId, paymentScreenshot
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
